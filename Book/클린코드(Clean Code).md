...
깨끗한 코드로 작성하며 표현력을 높이고 간결하게 정리한다. 테스트 API 를 구현해 도메인 특화언어를 만들면 테스트 코드를 짜기가 쉬워진다.<br>
<br>
